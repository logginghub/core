...
public class StringUtils {
    ...
    public static boolean contains(String string, String stringToSearchFor, boolean caseSensitive) {
        boolean contains;

        if (caseSensitive) {
            contains = string.contains(stringToSearchFor);
        } else {
            contains = indexOfIgnoreCaseSlow(string, stringToSearchFor) != -1;
        }

        return contains;
    }

    // jshaw - taken from http://stackoverflow.com/questions/1126227/indexof-case-sensitive
    public static int indexOfIgnoreCaseSlow(final String haystack, final String needle) {
        ...
        for (int i = 0; i < haystackLength; ++i) {
            ...
            int j = 0;
            int ii = i;
            while (ii < haystackLength && j < needleLength) {
    ...
    public static boolean containsIgnoreCase(String string, String stringToSearchFor) {
        boolean contains = false;

        if (string != null && stringToSearchFor != null) {
            int len = stringToSearchFor.length();
            int max = string.length() - len;
            for (int i = 0; i <= max && !contains; i++) {
                if (string.regionMatches(true, i, stringToSearchFor, 0, len)) {
                    contains = true;
                }
            }
        }
    ...
    public static int indexOfIgnoreCase(String source, String target) {
        return indexOfIgnoreCase(source.toCharArray(), 0, source.length(), target.toCharArray(), 0, target.length(), 0);
    }
    ...
    public static int indexOfIgnoreCase(char[] source,
        ...
        char firstLower = target[targetOffset];
        char firstUpper = target[targetOffset];
        ...
        for (int i = sourceOffset + fromIndex; i <= max; i++) {
            ...
            if (source[i] != firstLower && source[i] != firstUpper) {
                while (++i <= max && (source[i] != firstUpper && source[i] != firstLower))
                    ;
            }
            ...
            if (i <= max) {
                ...
                // If the target is only 1 char long we'll already be done
                if (j < end) {

                    boolean carryOnSearching = true;

                    int k = targetOffset + 1;

                    while (carryOnSearching) {

                        char targetLower = target[k];
                        char targetUpper = target[k];

                        if (targetLower >= 'A' && targetLower <= 'Z') {
                            targetLower += 'a' - 'A';
                        }

                        if (targetUpper >= 'a' && targetUpper <= 'z') {
                            targetUpper -= 'a' - 'A';
                        }

                        char sourceChar = source[j];

                        carryOnSearching = sourceChar == targetLower || sourceChar == targetUpper;
    ...
    public static int indexOfIgnoreCase(char[] source, char[] target) {
        return indexOfIgnoreCase(source, 0, source.length, target, 0, target.length, 0);
    }
