...
public class Logger {
    ...
    public final static int stats = 750;
    ...
    public static void loadProperties() {
        ...
        if (override != null) {
            internalDebug("logginghub.levels.properties variable (from system property or environment) detected : '{}'", override);
            ...
            if (!userSpecifiedFile.exists()) {
                ...
                if (read != null) {
                    ...
                    throw new IllegalArgumentException(StringUtils.format(
                            "logging.levels.properties property has been set to '{}', but we tried to find the file at '{}' and it wasn't found, " +
                            "and we tried to load it as a resource from the classpath, and that also failed. Please check your settings.",
                            override,
            ...
            internalDebug("No logginghub.levels.properties variable (from system property or environment) detected, using default search order");
        }
        ...
        if (settings == null) {
            ...
            internalDebug("Looking for '{}' in the working directory : '{}'", defaultFilename, workingDirectory.getAbsolutePath());
            if (workingDirectory.exists()) {
                ...
                internalDebug("Properties file not found, looking the user's home directory : '{}'", userHome.getAbsolutePath());
                if (userHome.exists()) {
                    internalDebug("Properties file found.");
                    settings = userHome.getAbsolutePath();
                } else {
                    // That's it, where else could we look?
                    internalDebug("Properties file not found in any of the usual places. We'll have to use the default configuration.");
                }
            }
        }
            ...
            if (propertiesInFile.exists()) {
                WorkerThread.everyNowDaemon("LoggingHub-LoggerConfigReader", 1, TimeUnit.SECONDS, new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (time != fileTime) {
                            internalDebug("Reloading properties from '{}' due to modification", propertiesInFile.getAbsolutePath());
                            resetLevels();
            ...
            for (String line : splitIntoLineList) {
                ...
                if (split.length == 2) {
                    ...
                    if (key.trim().startsWith("#")) {
                        ...
                        int level = parseLevel(value.trim());
                        internalDebug("Setting level '{}' = {}", key, level);

                        if (key.trim().equals("systemErrStream.level")) {
                            getSystemErrStream().setLevelFilter(level);
                        } else {
                            setLevel(key, level);
                        }
                    }
    ...
    public static SystemErrStream getSystemErrStream() {
    ...
    public void stats(String format, Object... objects) {
        if ((level < 0 && root.level <= stats) || (level >= 0 && level <= stats)) {
            log(stats, format, objects);
        }
    }
