...
//            currentBufferLock.writeLock().lock();
            ...
            try {
                ...
                if (originalWatermarkUsed == 0) {
                    ...
                    logger.warn("Serialised event was too large to fit into an empty block, so we are dropping it. You might want to consider increasing your block size (currently {}) if you have large events.",
...
//                currentBufferLock.writeLock().unlock();
    ...
    @Override public void extractEventsBetween(final List<LogEvent> matchingEvents, long start, long end) {
