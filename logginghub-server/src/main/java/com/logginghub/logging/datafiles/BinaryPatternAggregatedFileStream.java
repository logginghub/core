 ...
 */
public class BinaryPatternAggregatedFileStream implements Destination<PatternAggregation> {
    ...
    public BinaryPatternAggregatedFileStream(File folder, String name) {
        this.folder = folder;
        this.name = name;
    }

    public static void replay(File file, Destination<PatternAggregation> destination) {
            ...
            while ((read = channel.read(buffer)) != -1) {
                ...
                try {
                    while (buffer.hasRemaining()) {
                        buffer.mark();
                        PatternAggregation decoded = SofExpandingBufferSerialiser.read(buffer, sofConfiguration);
    ...
    public void send(PatternAggregation event) {
