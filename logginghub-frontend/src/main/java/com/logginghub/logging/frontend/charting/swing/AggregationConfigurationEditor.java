...
import com.logginghub.logging.frontend.charting.swing.ChartSeriesModelEditor.LabelIndexWrapper;
import com.logginghub.logging.frontend.charting.swing.ChartSeriesModelEditor.PatternModelWrapper;
...
public class AggregationConfigurationEditor extends JPanel {

    private static Logger logger = Logger.getLoggerFor(AggregationConfigurationEditor.class);
    private final JTextField aggregationName;
    ...
    private AggregationConfiguration model;
...
//    private final static LabelIndexWrapper blankWrapper = new LabelIndexWrapper(-1, "");
    ...
    public AggregationConfigurationEditor() {

        setName("AggregationConfigurationEditor");
        setLayout(new MigLayout("", "[][grow]", "[][][][][][][][][][][134.00,grow]"));

        setBackground(Color.white);
        setOpaque(true);

        JLabel aggregationNameLabel = new JLabel("Aggregation name");
        add(aggregationNameLabel, "cell 0 0,alignx trailing");
        this.aggregationName = new JTextField();
        this.aggregationName.setName("Aggregation name");
        add(aggregationName, "cell 1 0,growx");
        ...
        add(patternCombo, "cell 1 1,growx");
        ...
        if (index != -1) {
            ...
            if (patternWrapper != null) {
                ...
                for (PatternModel patternModel : patternModels) {
                    if (patternModel.getPatternID().get() == patternWrapper.getPatternModel().getPatternID().get()) {
    ...
    public void bind(final NewChartingController controller, final AggregationConfiguration model, final NewChartingModel chartingModel) {
        this.controller = controller;
        this.model = model;
        this.chartingModel = chartingModel;
        binder = new Binder2();

        binder.bind(model.getName(), aggregationName);
...
//        binder.bind(model.getGenerateEmptyTicks(), generateEmptyTicks);
    ...
    protected void setupLabels(AggregationConfiguration model, NewChartingModel chartingModel) {
