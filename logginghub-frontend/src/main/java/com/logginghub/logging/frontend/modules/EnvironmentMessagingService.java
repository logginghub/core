...
public interface EnvironmentMessagingService {
    ...
    /**
     * Send a request message to this environment; any response will be sent back to the destination
     * provided - if and when it arrives.
     */
    <T> void send(RequestResponseMessage request, Destination<LoggingMessage> destination);

    /**
     * Send a request that might result in multiple messages being sent back. The return object is
     * used to in the called to endStreaming to remove the listener when you are done.
     * 
     * @param request
     * @param destination
     * @return
