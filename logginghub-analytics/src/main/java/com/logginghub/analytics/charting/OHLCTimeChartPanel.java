 ...
 * 
 */
public class OHLCTimeChartPanel extends JPanel implements ChartPanelInterface {
    ...
    public OHLCTimeChartPanel() {

        chart = ChartFactory.createHighLowChart("High-Low-Open-Close Demo", "Time", "Price", ohlcSeriesCollection, true);
    ...
    @Override public void createSeries(String seriesName, List<Pair<Comparable, Number>> data) {
        ...
        //
        // for (Pair<Comparable, Long> dataPoint : data) {
        // seriesForSource.add((Double) dataPoint.getA(), dataPoint.getB());
        // }
        //
    ...
    @Override public void addOHLCSeries(String category, List<Pair<Integer, OHLCValue>> values) {
        ...
        OHLCSeries series = new OHLCSeries(category);

        for (Pair<Integer, OHLCValue> pair : values) {
            RegularTimePeriod time = new FixedMillisecond(pair.getA());
            series.add(time, pair.getB().open, pair.getB().high, pair.getB().low, pair.getB().close);
        }
